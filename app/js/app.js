$(document).ready(function() {
    ...
    var formBalanceFieldText = $(".form-balance__field-text");

    $(".form-balance__field").on("input", function() {
        var value = $(this).val();
        if ($(this).val().length == 0) {
            formBalanceFieldText.html("0");
        } else {
            formBalanceFieldText.html(value);
        }
        var width = formBalanceFieldText.outerWidth();
        $(this).css("width", width + "px");
    });

    $(".form-balance__select__item").on("click", function() {
        var count = $(this).text();
        formBalanceFieldText.html(count);
        $(".form-balance__field").val(count);
        var width = formBalanceFieldText.outerWidth();
        $(".form-balance__field").css("width", width + "px");
    });

    $("#modal-promocode").on("change", function() {
        $(".form-balance__promocode__content").toggleClass("active");
    });

    $(".offcanvas-btn-open , .offcanvas-btn-close").on("click", function() {
        $("#offcanvas").toggleClass("active");
    });

    $("#scroll-to-items").on("click", function(event) {
        event.preventDefault();
        var scroll = $("#profile_inventory_block").offset().top;
        $("html, body").animate({ scrollTop: scroll }, 700);
    });
    //fix collapse max-width 1199px
    // $('#slide-navbar-collapse').on('show.bs.collapse', function(){
    //     var that = $(this);
    //     setTimeout(function(){
    //         that.attr('style', 'display:block !important');
    //     }, 150);
    // });
    // $('#slide-navbar-collapse').on('shown.bs.collapse', function(){
    ...
    $("#single_select").multiselect({
        templates: {
            li:
                '<li><a href="javascript:void(0);"><label class="pl-2"></label></a></li>'
        },
    ...
    $("#paysum").keyup(function() {
        var paysum = $(this).val();
        var tasty_promo_test = (($("#bpercent").val() * paysum) / 100).toFixed(
            0
        );
        $("#paybag__get-promo").html(tasty_promo_test);
        var bonus = 0;

        if (paysum >= 5000) bonus = ($("#paysum").val() * 15) / 100;
        else if (paysum >= 3000) bonus = ($("#paysum").val() * 10) / 100;
        else if (paysum >= 1000) bonus = ($("#paysum").val() * 5) / 100;

        $("#payget").html(
            (
                parseFloat(bonus) +
                parseFloat(paysum) +
                parseFloat(tasty_promo_test)
            ).toFixed(0)
        );
    ...
    !(function($) {
        ...
        TabCollapse.prototype.checkState = function() {
            if (this.$tabs.is(":visible") && this._accordionVisible) {
                ...
                this.$accordion.is(":visible") &&
                !this._accordionVisible
            ) {
                this.showAccordion();
        ...
        TabCollapse.prototype.showTabs = function() {
            ...
            var $panelBodies = this.$accordion.find(
        ...
        TabCollapse.prototype.showAccordion = function() {
            ...
            if (this.options.updateLinks) {
                ...
                var $selector = this.$accordion.find(
                    ".js-tabcollapse-panel-body"
                );
                $selector
        ...
        TabCollapse.prototype._createAccordionGroup = function(
            ...
            if (!tabSelector) {
                ...
                tabSelector =
                    tabSelector && tabSelector.replace(/.*(?=#[^\s]*$)/, ""); //strip for ie7
            }
